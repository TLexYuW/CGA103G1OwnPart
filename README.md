...
```
   - 點擊`查詢主辦之活動列表`，可以列出該會員自己所創建的主辦活動編號及標題名稱
![L](https://i.imgur.com/oLlJnYT.png)

   - 若無任何主辦活動，則回傳訊息`目前您無任何主辦活動`提示
![L](https://i.imgur.com/B3VR6bu.png)

   - 於欄位輸入主辦活動編號，並點擊`搜尋`，若輸入編號錯誤，則回傳`查無此活動編號`提示
![L](https://i.imgur.com/HMmF2tr.png)

   - 活動編號正確，即可以列出此編號活動所有資料
   - 再點擊`修改活動條件`，即可修改活動資料
   - 接著點選`儲存`再按`送出更新揪團資料`，修改成功則會回傳成功訊息
![L](https://i.imgur.com/j7MOUaj.png)
![L](https://i.imgur.com/ftoDtr1.png)
...
- 點選列表內其中之一活動，進入該活動詳細頁面，
	...
	![L](https://i.imgur.com/eTeqs2D.png)
	
	- 若已加入過或此活動為該會員所創建 (後端驗證)，則回傳訊息告知`你已經加入過此活動`
	![L](https://i.imgur.com/NqrwCkg.png)
	
	- 如活動人數已達最多人數限制 (後端驗證)，則回傳訊息告知`加入失敗，活動已超過最大人數限制，無法加入`
	![L](https://i.imgur.com/NSmqtOG.png)
